...
class ATuple:
    ...
    def lineage(self,root_span=None)-> List[ATuple]:
        # YOUR CODE HERE (ONLY FOR TASK 1 IN ASSIGNMENT 2)
        resultTup = self.operator.lineage(tuples=[self],root_span=root_span)
        return resultTup
        pass
...
class Scan(Operator):
    ...
    def __init__(self, filepath,bsize,filter=None, track_prov=False,
        ...
        with open(self.filepath) as file:
            ...
            for line in freader:
                ...
                lineCount+=1
                # if self.track_prov == True and self.propagate_prov == True:
                #     Mytuple = ATuple(temptuple, metadata=self.identifier + str(lineCount), operator=self)
                # # else:
                #     Mytuple = ATuple(temptuple)
    ...
    #             batch_tuple.append(Mytuple)
    #             self.totaltuple.append(Mytuple)
    #             # if last_line_number == freader.line_num:
    #             #     yield batch_tuple
    ...
    def get_next(self):
        ...
        try:
            ...
            # print("Stop")
            return None
        # if self.counter > len(self.totaltuple)-1:
        #     return None
        # else:
        #     output = self.totaltuple[self.counter]
        #     self.counter +=1 
        #     return output
...
class Join(Operator):
    ...
    # Returns the lineage of the given tuples
    def lineage(self, tuples,root_span):
        ...
        with tracer.start_span('JoinL', child_of=root_span) as span:
            resultlist = []
...
class Project(Operator):
    ...
    def lineage(self, tuples,root_span=None):
        # print(self.input_to_output)
        config = Config(
            config={  # usually read from some yaml config
                'sampler': {
        ...
        # this call also sets opentracing.tracer
        tracer = config.new_tracer()
        with tracer.start_span('projectL', child_of=root_span) as span:
            resultlist = []
            for p in tuples:
                # print(p)
                for joinedTuple in self.input_to_output:
                    ...
                    if p == joinedTuple[0]:
                        ...
                        result = joinedTuple[1][0].lineage(span.context)
                        # print(result)
                        resultlist.extend(result)
...
class OrderBy(Operator):
    ...
    def lineage(self, tuples,root_span=None):
        ...
        with tracer.start_span('OrderByLineage') as span:
            for r in tuples:
