...
class Scan(Operator):
    ...
    def __init__(self, filepath,bsize,filter=None, track_prov=False,
        ...
        self.identifier =''
        self.counter = 0
        # self.tracer = init_tracer('Recommd')
        with open(self.filepath) as file:
            freader =csv.reader(file, delimiter=' ')
            # last_line_number = row_count(filepath)
            batch_tuple=[]
            lineCount = 0
            for line in freader:
            ...
            self.input_iter = iter(batch_tuple)
                # print(self.totaltuple)
        pass
    ...
    def get_next(self,root_span):
        ...
        with tracer.start_span('scan', child_of=root_span) as span:
            result = []
            try:
                nextt = next(self.input_iter)
                result.append(nextt)
                    # print(result)
                return result
...
class Join(Operator):
    ...
    def get_next(self,root_span):
        ...
        # this call also sets opentracing.tracer
        tracer = config.new_tracer()
        with tracer.start_span('Join',child_of=root_span) as span:
        # YOUR CODE HERE
            allLeftTuple=[]
            while True:
                # tempLeft = ray.get(self.left_input.get_next.remote())
                getleft = self.left_input.get_next.remote(span.context)
                tempLeft = ray.get(getleft)
            ...
            # right_batch = ray.get(self.right_input.get_next.remote())
            getright = self.right_input.get_next.remote(span.context)
            right_batch = ray.get(getright)
        ...
        # allLeftTuple=[]
        # while True:
        #     getleft = self.left_input.get_next.remote()
        #     tempLeft = ray.get(getleft)
        #     # tempLeft = self.left_input.get_next()
        #     if tempLeft == None:
        #         break
        ...
        #     left.setdefault(tuple_k.tuple[self.left_join_attribute],tuple_k)
        #
        # getright = self.right_input.get_next.remote()
        # right_batch = ray.get(getright)
        # # right_batch = self.right_input.get_next()
        # if right_batch == None:
        #     return None
        ...
        for i in tuples:
            for k in self.input_to_output:
                if i == k[0]:
                    ...
                    resultlist.extend(result)
                    # result = k[2].where(att_index=att_index)
...
class Project(Operator):
    ...
    def get_next(self,root_span):
        ...
        with tracer.start_span('Project',child_of=root_span) as span:
            result = []
            # getjoined = self.input.get_next.remote(span.context)
            # joined_tuple = ray.get(getjoined)
            # getjoined =
            joined_tuple = ray.get(self.input.get_next.remote(span.context))
        ...
        # result = []
        # getjoined = self.input.get_next.remote()
        # joined_tuple = ray.get(getjoined)
        # # joined_tuple = self.input.get_next()
        # if joined_tuple == None:
        #     return None
    ...
    # Returns the li    neage of the given tuples
    def lineage(self, tuples):
        # print(self.input_to_output)
        resultlist = []
        for p in tuples:
...
class GroupBy(Operator):
    ...
    def get_next(self,root_span):
        ...
        with tracer.start_span('GroupBy',child_of=root_span) as span:
            while True:
                projectTuple = ray.get(self.input.get_next.remote(span.context))
                # getprojectTuple = self.input.get_next.remote(span.context)
                # projectTuple = ray.get(getprojectTuple)
        ...
        # while True:
        #     getprojectTuple = self.input.get_next.remote()
        #     projectTuple = ray.get(getprojectTuple)
        #     # projectTuple = projectOp.get_next()
        #     if projectTuple == None:
        #         break
...
class OrderBy(Operator):
    ...
    def get_next(self,root_span):
        config = Config(
            ...
            service_name='OrderByOp',
            validate=True,
        )
        # this call also sets opentracing.tracer

        tracer = config.new_tracer()
        # tracer4 = init_tracer('Recommd')
        with tracer.start_span('OrderBy',child_of=root_span) as span:
            tuplelist = []
            while True:
                groupTuple = ray.get(self.input.get_next.remote(span.context))
                # getGroupTuple = self.input.get_next.remote(span.context)
                # groupTuple = ray.get(getGroupTuple)
        ...
        # tuplelist = []
        # while True:
        #     getGroupTuple = self.input.get_next.remote()
        #     groupTuple = ray.get(getGroupTuple)
        #     # groupTuple = self.input.get_next()
        #     if groupTuple == None:
        #         break
...
class TopK(Operator):
    ...
    def get_next(self,root_span):
        ...
        tracer = config.new_tracer()
        # tracer5 = init_tracer('Recommd')
        with tracer.start_span('TopK',child_of=root_span) as span:
            result = []
            orderedTuple = ray.get(self.input.get_next.remote(span.context))
            # getOrderedTuple = self.input.get_next.remote(span.context)
            # orderedTuple = ray.get(getOrderedTuple)
        ...
        # result = []
        # getOrderedTuple = self.input.get_next.remote()
        # orderedTuple = ray.get(getOrderedTuple)
        # # orderedTuple = self.input.get_next()
        # for index,element in enumerate(orderedTuple):
        #     if index==self.k:
...
class Select(Operator):
    ...
    def get_next(self,root_span):
        ...
        # this call also sets opentracing.tracer
        tracer = config.new_tracer()
        with tracer.start_span('select',child_of=root_span) as span:

            # scantuple = ray.get(self.input.get_next.remote(span.context))
            gettuple = self.input.get_next.remote(span.context)
            scantuple = ray.get(gettuple)
            # scantuple = self.input.get_next.remote()
            filtered_tuple = []
            if scantuple == None:
                return None
        ...
        # gettuple = self.input.get_next.remote()
        # scantuple = ray.get(gettuple)
        # # scantuple = self.input.get_next.remote()
        # filtered_tuple = []
        # if scantuple == None:
...
if __name__ == "__main__":
    ...
    #Avg test
    logger.info("Assignment #1 Task 1")



    # scanFriendOp = Scan.remote(toyfriend, 1)
    # friendFilter = Filter(1190 , 0)
    # friendSelectOp = Select.remote(scanFriendOp, friendFilter)
    # scanMovieOP = Scan.remote(toymovie, 1)
    # movieFilter = Filter(0, 1)
    # movieSelectOp = Select.remote(scanMovieOP, movieFilter)
    # joinOp = Join.remote(friendSelectOp, movieSelectOp, left_join_attribute=1, right_join_attribute=0)
    # projectOp = Project.remote(joinOp, [1, 3, 4])
    # allProjectTuple = []
    # while True:
    #     getprojectTuple = projectOp.get_next.remote()
    #     projectTuple = ray.get(getprojectTuple)
    #     if projectTuple == None:
    #         break
    ...
    # theTuple = scanFriendOp.get_next.remote()
    # print(ray.get(theTuple))
    # theTuple2 = scanFriendOp.get_next.remote()
    # print(ray.get(theTuple2))
    ...
    # logger.info("Assignment #1 Task 2")
    # scanFriendOp2 = Scan.remote(toyfriend, 1)
    # friendFilter = Filter(1190, 0)
    # friendSelectOp2 = Select.remote(scanFriendOp2, friendFilter)
    # scanMovieOp2 = Scan.remote(toymovie, 1)
    # joinOp2 = Join.remote(friendSelectOp2, scanMovieOp2, left_join_attribute=1, right_join_attribute=0)
    # projectOp2 = Project.remote(joinOp2, [1, 3, 4])
    # groupOp = GroupBy.remote(input=projectOp2,agg_fun=AVG)
    # orderByOp = OrderBy.remote(input=groupOp,comparator=customComp,ASC=False)
    # topKOp = TopK.remote(orderByOp,1)
    # getresult= topKOp.get_next.remote()
    # result = ray.get(getresult)
    ...
    def task2():
        ...
        # this call also sets opentracing.tracer
        tracer = config.new_tracer()
        with tracer.start_span('Task2') as span:
            scanFriendOp2 = Scan.remote(toyfriend, 1)
            friendFilter = Filter(1190, 0)
            friendSelectOp2 = Select.remote(scanFriendOp2, friendFilter,track_prov=True)
            scanMovieOp2 = Scan.remote(toymovie, 1)
            joinOp2 = Join.remote(friendSelectOp2, scanMovieOp2, left_join_attribute=1, right_join_attribute=0,track_prov=True)
            projectOp2 = Project.remote(joinOp2, [1, 3, 4],track_prov=True)
            groupOp = GroupBy.remote(input=projectOp2,agg_fun=AVG,track_prov=True)
            orderByOp = OrderBy.remote(input=groupOp,comparator=customComp,ASC=False,track_prov=True)
            topKOp = TopK.remote(orderByOp,1)
            getresult= topKOp.get_next.remote(span.context)
            result = ray.get(getresult)
    ...
    # logger.info("Assignment #1 Task 2 track_prov")
    # scanFriendOp2 = Scan.remote(toyfriend, 1)
    # friendFilter = Filter(1190, 0)
    # friendSelectOp2 = Select.remote(scanFriendOp2, friendFilter,track_prov=True)
    # scanMovieOp2 = Scan.remote(toymovie, 1)
    ...
    # logger.info("Assignment #2 task 1")
    # scanFriendOp = Scan.remote(toyfriend, 1)
    # friendFilter = Filter(1190, 0)
    # friendSelectOp = Select.remote(scanFriendOp, friendFilter)
    # scanMovieOp = Scan.remote(toymovie, 1)
    # joinOp = Join.remote(friendSelectOp, scanMovieOp, left_join_attribute=1, right_join_attribute=0,track_prov=True)
    # projectOp = Project.remote(joinOp, [1, 3, 4],track_prov=True)
    # groupOp = GroupBy.remote(input=projectOp, agg_fun=AVG,track_prov=True)
    # orderByOp = OrderBy.remote(input=groupOp, comparator=customComp, ASC=False, track_prov=True)
    # topKOp = TopK.remote(orderByOp, 1)
    # result = ray.get(topKOp.get_next.remote())
    # print(result[0])
    # print(result[0].lineage())
    ...
    # YOUR CODE HERE
    # logger.info("Assignment #2 task 2")
    # scanFriendOp = Scan(toyfriend, 1)
    # friendFilter = Filter(1190, 0)
    # friendSelectOp = Select(scanFriendOp, friendFilter)
    # scanMovieOp = Scan(toymovie, 1)
    ...
    # YOUR CODE HERE
    # logger.info("Assignment #2 task 3")
